...
public class Board
{
    ...
    private bool m_createdBoard = false;

    private GameManager.eTheme m_theme;

    public Board(Transform transform, GameSettings gameSettings)
    {
        m_theme = gameSettings.Theme;
    ...
    private void CreateBoard()
    {
        if (!m_createdBoard)
        {
            ...
            var fastPool = FastPoolManager.GetPool(prefabBG);
            ...
            for (int x = 0; x < boardSizeX; x++)
            {
                for (int y = 0; y < boardSizeY; y++)
                {
                    ...
                    GameObject go = fastPool.FastInstantiate();
        ...
        if (!m_createdBoard)
        {
            for (int x = 0; x < boardSizeX; x++)
            {
                for (int y = 0; y < boardSizeY; y++)
                {
                    ...
                    NormalItem item = new NormalItem(m_theme);
    ...
    internal void FillGapsWithNewItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                var fitType = GetFitType(cell);
                item.SetType(fitType);
                //Cần tìm type phù hợp
    ...
    private NormalItem.eNormalType GetFitType(Cell cell)
    {
        int min_item = 999;
        NormalItem.eNormalType fitType = NormalItem.eNormalType.TYPE_ONE;
        
        //Cac type xung quanh
        var aroundCells = cell.AroundCells;
        List<NormalItem.eNormalType> aroundType = new List<NormalItem.eNormalType>();
        foreach (var aroundCell in aroundCells)
        {
            var type = ((NormalItem)aroundCell.Item).ItemType;
            if(!aroundType.Contains(type)) aroundType.Add(type);
        }
        
        //trả về type có số lượng items ít nhất
        foreach (NormalItem.eNormalType type in Enum.GetValues(typeof(NormalItem.eNormalType)))
        {
            if (!aroundType.Contains(type))
            {
                int itemCounts = GetItemCounts(type);
                if (itemCounts < min_item)
                {
                    min_item = itemCounts;
                    fitType = type;
                }
    ...
    /// <summary>
    /// Lấy số lượng item kiểu type trong board
    /// </summary>
    /// <param name="type"></param>
    /// <returns></returns>
    /// <exception cref="NotImplementedException"></exception>
    private int GetItemCounts(NormalItem.eNormalType type)
    {
        int itemCounts = 0;
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                if (!cell.IsEmpty)
                {
                    ...
                    if (item.ItemType == type) itemCounts++;
                }
